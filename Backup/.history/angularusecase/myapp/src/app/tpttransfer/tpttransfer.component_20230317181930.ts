...
export class TpttransferComponent {
  ...
  transfer(account_no:number){
    ...
    this.http.put(`http://localhost:52000/thirdparty/transaction/${adder_id}/${account_no}`,{'amount':this.depositAmount}).subscribe(data => {
      data=data
...
send(transaction: any) {
  // Send transaction.amount to the server
  transaction.amount = false; // hide the input field and "Send" button
}
